...
import { BranchesPathsCalculator } from "./branches-paths";
import { booleanOptionOr, numberOptionOr } from "./utils";
import { Orientation } from "./orientation";
import { GitgraphUserApi, } from "./user-api/gitgraph-user-api";
export { GitgraphCore };
class GitgraphCore {
    get isHorizontal() {
    ...
    get isReverse() {
        return (this.orientation === Orientation.HorizontalReverse ||
    ...
    constructor(options = {}) {
        ...
        this.branchesOrderFunction =
            typeof options.compareBranchesOrder === "function"
                ...
                : undefined;
        this.branchLabelOnEveryCommit = booleanOptionOr(options.branchLabelOnEveryCommit, false);
    }
    /**
    ...
    computeRenderedCommits() {
        ...
        const branchesOrder = new BranchesOrder(commitsWithBranches, this.template.colors, this.branchesOrderFunction);
    ...
    computeRenderedBranchesPaths(commits) {
        return new BranchesPathsCalculator(commits, this.branches, this.template.commit.spacing, this.isVertical, this.isReverse, () => createDeletedBranch(this, this.template.branch, () => this.next())).execute();
    }
    /**
    ...
    computeBranchesColor(commits, branchesPaths) {
        const branchesOrder = new BranchesOrder(commits, this.template.colors, this.branchesOrderFunction);
